...
module.controller('MapCtrl', function ($scope, $http) {
    ...
    function ready(error, topo, json) {
        ...
        var center = d3.geoCentroid(topojson.feature(topo, topo.objects.hokkaido))
        //console.log(center);
        ...
        var quantizeNegative = d3.scaleQuantize()
            .domain([min, 0])
            .range(d3.range(10).reverse().map(function(idx){
                return idx;
            }));

        var quantizePositive = d3.scaleQuantize()
            .domain([0, max])
        ...
        var getRangePoints = function () {
            ...
            console.log(o(1))
            var rangeTxt = []
            for (var i = 1; i <= raw_dataset.length; i++) {
                if (i == 1) {
                    rangeTxt.push(mathRound(o(i),2) + " ~");
                    continue
                }
                rangeTxt.push(mathRound(o(i),2) + " ~ " + mathRound(o(i-1),2));
            }
            var o1 = d3.scalePoint()
                .domain(raw_dataset)
                .range([0, min]);
            console.log(min)

            for (var i = 1; i <= raw_dataset.length; i++) {
                if (i==raw_dataset.length) {
                    rangeTxt.push(mathRound(o1(i),2) + " ~");
                    continue;
                }
                rangeTxt.push(mathRound(o1(i),2) + " ~ " + mathRound(o1(i + 1),2));
            }
            return rangeTxt;
        }
